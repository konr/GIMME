# -*- coding: utf-8 -*-
$atribs=["title","id","artist","album", "duration","starred","url","tracknr", "timesplayed"]
...
module Xmms
  class Collection
    ...
    def to_sexp; $parser.to_sexp(self.to_a); end
...
class GIMME
  ...
  def insert_next (id)
    @async.playlist("_active").current_pos.notifier do |pos|
      @async.playlist("_active").insert_entry(pos[:position]+1,id).notifier
    end
  end
